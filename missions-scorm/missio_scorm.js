...
function showElegantAlert(msg) {
    ...
    alertPopup.style.width = '100vw';
    alertPopup.style.height = '100vh';
    alertPopup.style.background = 'rgba(0,0,0,0.3)';
    alertPopup.style.display = 'flex';
    alertPopup.style.alignItems = 'center';
    alertPopup.style.justifyContent = 'center';
    alertPopup.style.zIndex = '10000';
    var alertBox = document.createElement('div');
    alertBox.style.background = 'white';
    alertBox.style.padding = '32px';
    alertBox.style.borderRadius = '12px';
    alertBox.style.boxShadow = '0 2px 16px rgba(0,0,0,0.3)';
    ...
    closeBtn.onclick = function() {
        if (alertPopup && alertPopup.parentNode) alertPopup.parentNode.removeChild(alertPopup);
    };
    alertBox.appendChild(document.createElement('br'));
    alertBox.appendChild(closeBtn);
    alertPopup.appendChild(alertBox);
...
function botoPausa() { 
    switch(estat) {
        case 0:    // no assignada
            showElegantAlert("Encara no teniu cap missió assignada. Incorporeu-vos primer a una missió!");
            break;
        case 1:    // assignada
            if (comprova_inici()) {
                estat = 2;
            ...
            break;
        case 2:    // iniciada
            showElegantConfirm("Segur que voleu avortar la missió? Haureu de tornar a iniciar-la i completar-la ...", function(result) {
                if (result) {
                    restaura();
                }
            });
...
function comprova_inici() {
    if (disSensors==0) {
    showElegantAlert("No heu incorporat cap sensor al satèl·lit, no podreu completar la missió assignada.");
        return false;
    } else {
        if (!row_mis[nM] || !row_mis[nM][9]) {
            showElegantAlert("Error: No s'ha pogut obtenir la informació de la missió. Torna a assignar una missió.");
            return false;
        }
        ...
        if (equipat) {
            ...
            var avís = "No heu incorporat cap dels sensors recomenats per completar la missió assignada:<br><br>"
            for(var j=0;j<possibles.length;j++) {
                if (row_sen[possibles[j]]) {
                    avís+= "· " + row_sen[possibles[j]][0] + "<br>"
                } else {
                    avís+= "· Sensor desconegut (id: " + possibles[j] + ")<br>"
                }
            }
            showElegantAlert(avís);
            return false;
        }
    }
}
...
function addGraph() {
    ...
    // Primer comprovem si la missió està completada
...
function botoAfegirEstacio() {
    showElegantAlert("La nova estació s'ha ubicat provisionalment en el mapa.<br>Pots canviar la ubicació de les estacions arrossegant-les<br>amb el ratolí.");
...
function botoAfegirZona() {
    showElegantAlert("La nova zona d'estudi s'ha ubicat provisionalment en el mapa.<br>Pots canviar la ubicació de les zones d'estudi arrossegant-les<br>amb el ratolí.");
...
function botoInfoSensors() {
    ...
    showElegantAlert("No es poden treure o afegir sensors quan el satèl·lit ja està orbitant...");
}
}	
...
function botoMostraInstruccions() {
    $("#modalHeader").html("Instruccions");	
    $("#missatge").html('<object type="text/html" data="missio_scorm-instruccions.html" width="100%" height="450px"></object>');
    $("#finestraInfo").css({"visibility":"visible"});
    $("#finestraInfo").modal('show');
}
